...
end.sig.up <- end.sig[end.sig$logFC > 0, "Name"]; length(end.sig.up)  # 27163
end.sig.dn <- end.sig[end.sig$logFC < 0, "Name"]; length(end.sig.dn)  # 20589
...
res.sig.up <- res.sig[res.sig$logFC > 0, "Name"]; length(res.sig.up)  # 27163
res.sig.dn <- res.sig[res.sig$logFC < 0, "Name"]; length(res.sig.dn)  # 20589

#. overlaps ----
overlap.all <- intersect(end.all$Name, res.all$Name); length(overlap.all)  # 41652
overlap.sig <- intersect(end.sig$Name, res.sig$Name); length(overlap.sig)  # 41652
...
for (i in seq_along(overlap.list)) {
    cat("####", names(overlap.list[i]), "####", "\n")
    entrezids.list[[i]] <- getMappedEntrezIDs(
                                sig.cpg = overlap.list[[i]],
                                array.type = "450K",
                                )
    print(head(entrezids.list[[i]]$sig.eg))
    cat("length:", length(entrezids.list[[i]]$sig.eg), "\n\n")
    }
bkgd.entrez <- entrezids.list[[1]]$sig.eg; head(bkgd.entrez)
sig.entrez <- entrezids.list[[2]]$sig.eg; head(sig.entrez)
up.entrez <- entrezids.list[[3]]$sig.eg; head(up.entrez)
dn.entrez <- entrezids.list[[4]]$sig.eg; head(dn.entrez)
