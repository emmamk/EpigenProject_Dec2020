...
pD$timeshort <- str_sub(pD$timepoint, 1, 1)
pD$sample <- paste0("S", pD$sample)
pD$ID <- paste(pD$sample, pD$timeshort, sep=".")
head(pD)
tail(pD)
...
#. . . Ref Files ----
library(readxl)
ex1 = read_excel(paste(dataDir, "i450k_ns.xlsx", sep="/"), sheet = 1, stringsAsFactors = FALSE)
ex2 = read_excel(paste(dataDir, "i450k_pl.xlsx", sep="/"), sheet = 1, stringsAsFactors = FALSE)
ex3 = read_excel(paste(dataDir, "i450k_pl.xlsx", sep="/"), sheet = 2, stringsAsFactors = FALSE)
...
# factor of interest
timepoint <- factor(pDmale$timepoint)
relevel(timepoint, "before")
# individual effect that we need to account for
individual <- factor(pDmale$sample)

# design matrix
design <- model.matrix(~0+timepoint+individual, data=pDmale)
...
contMatrix <- makeContrasts(before-after,
                            levels=design)
contMatrix
...
sapply(rownames(DMPs)[1:4], function(cpg){
    plotCpg(bVals, cpg=cpg, pheno=pDmale$timepoint, ylab = "Beta values")
})
...
groups <- pal[1:length(unique(pDmale$timepoint))]
names(groups) <- levels(factor(pDmale$timepoint))
cols <- groups[as.character(factor(pDmale$timepoint))]
...
#### Ex. of Differential Variability ####
# load data
# Ref: https://bioconductor.org/packages/devel/workflows/vignettes/methylationArrayAnalysis/inst/doc/methylationArrayAnalysis.html#differential-variability
...
keep <- !(featureNames(age.mSetSqFlt) %in% annEPIC$Name[annEPIC$chr %in% 
                                                            c("chrX","chrY")])
# visualise
age.pal <- brewer.pal(8,"Set1")
par(mfrow=c(1,2))
plotMDS(getM(age.mSetSqFlt), top=1000, gene.selection="common", 
        col=age.pal[factor(age.pD$timepoint)], labels=age.targets$Sex, 
        main="With Sex CHR Probes")
legend("topleft", legend=levels(factor(age.pD$timepoint)), 
       text.col=age.pal)

plotMDS(getM(age.mSetSqFlt[keep,]), top=1000, gene.selection="common", 
        col=age.pal[factor(age.pD$timepoint)], labels=age.targets$Sex, 
        main="Without Sex CHR Probes")
legend("top", legend=levels(factor(age.pD$timepoint)),
       text.col=age.pal)
...
design <- model.matrix(~factor(age.pD$timepoint))
...
sapply(rownames(topDV)[1:4], function(cpg){
    plotCpg(age.bVals, cpg=cpg, pheno=age.pD$timepoint, 
            ylab = "Beta values")
})
...
par(mfrow=c(1,1))
a = cellCounts[age.pD$timepoint == "NewBorns",]
b = cellCounts[age.pD$timepoint == "OLD",]
