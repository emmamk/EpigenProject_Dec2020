...
overlap.sig <- intersect(end.sig$UCSC_RefGene_Name, res.sig$UCSC_RefGene_Name); length(overlap.sig)  # 7537
...
ego.up <- clusterProfiler::enrichGO(
    ...
    pvalueCutoff = 0.05,
    readable = TRUE)
head(ego.up,10); nrow(ego.up) # 399
dotplot(ego.up, split="ONTOLOGY") + facet_grid(ONTOLOGY~., scale="free")


ego.dn <- clusterProfiler::enrichGO(
    ...
    readable = TRUE)
head(ego.dn,10)
dotplot(ego.dn, split="ONTOLOGY") + facet_grid(ONTOLOGY~., scale="free")
